...
- **`wdid config` commands live in `src/config-cli.ts`.** The `FIELDS` registry is the single source of truth for field metadata (type, secret, settable, nested). When adding a new config key in `src/config.ts`, also register it in `FIELDS` — otherwise `wdid config set <new-key>` will reject it as unknown. Type-aware parsing in `parseValue` uses this registry; secret masking in `renderConfigList` / `renderSingleValue` uses it; nested dotted access is gated by `nested: true` (only `togglProjects` has this so far). The commands only ever read/write the **global** config (`~/.config/wdid/config.json`); repo-level configs are out of scope on purpose to keep the mental model simple.
